...
public class WebSocketTopicInterceptorTest {
    ...
    private User user;
    private Group group;
    private Group unauthorizedGroup;
    ...
    public void addUserAndGroupToDatabase() {
        ...
        Group unauthorizedGroep = new Group("UnauthorizedGroup");

        this.user = this.userRepository.save(user);
        this.group = this.groupRepository.save(group);
        this.unauthorizedGroup = this.groupRepository.save(unauthorizedGroep);
    ...
    public void test_websocketWithUnauthoziredUser_shouldNotReceiveMessageFromTheServer() throws Exception {
        Assertions.assertThrows(Exception.class, () -> {
            ...
            StompSession stompSession = this.getStompSession();
            stompSession.subscribe(WEBSOCKET_TOPIC + "/" + unauthorizedGroup.getUuid(), new DefaultStompFrameHandler(completableFuture));

            sendMessage(this.unauthorizedGroup.getUuid().toString());

            completableFuture.get(5, TimeUnit.SECONDS);
        });
    }

    private void sendMessage(String groupUuid) {
        Date date = new Date();
        Timestamp createdAt = new Timestamp(date.getTime());
        Message message = new Message(this.group, this.user, "Hello World", createdAt);

        this.simpMessagingTemplate.convertAndSend(WEBSOCKET_TOPIC + "/" + groupUuid, message);
    }

    private StompSession getStompSession() throws Exception {
        WebSocketHttpHeaders httpHeaders = new WebSocketHttpHeaders();
        httpHeaders.add(WebSocketHttpHeaders.AUTHORIZATION, "Bearer token");

        StompSession stompSession = webSocketStompClient
                .connect(WEBSOCKET_URI, httpHeaders, new StompSessionHandlerAdapter() {
